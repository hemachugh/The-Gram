...
exports.getPostsOfFollowing = catchAsync(async (req, res, next) => {
  console.log("posts checkinh" + req.user.id);
  const user = await User.findById(req.user._id);
...
exports.getPostsForUserAndFollowing = catchAsync(async (req, res, next) => {
  console.log("Fetching posts for user and their following: " + req.user.id);
  const user = await User.findById(req.user._id);

  const currentPage = Number(req.query.page) || 1;
  const postsPerPage = 4; // You can adjust the number of posts per page
  const skipPosts = postsPerPage * (currentPage - 1);

  // Include the user's ID along with the IDs of users they are following
